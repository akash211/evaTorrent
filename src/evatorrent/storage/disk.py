...
class DiskWriter:
    """Handles writing verified pieces to the file system."""
    ...
    def _init_paths(self) -> None:
        ...
        for f in self.files:
            ...
            if not file_path.exists():
                # Pre-allocate or create empty file
    ...
    def write_piece(self, piece_index: int, data: bytes) -> None:
        ...
        for f in self.files:
            ...
            if piece_offset < file_end and piece_end > file_start:
                # Overlap boundaries within global torrent offset
                overlap_start = max(piece_offset, file_start)
                overlap_end = min(piece_end, file_end)

                # Offsets within the piece data and file
